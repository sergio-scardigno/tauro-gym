...
export default function StickyWhatsAppBar() {
  ...
  useEffect(() => {
    function handleScroll() {
      ...
      const isInContact = contactSection
        ? scrollTop >= contactSection.offsetTop - 100
        : false;
      const isPastHero = heroSection
        ? scrollTop >= heroSection.offsetTop + heroSection.offsetHeight * 0.5
        : scrollTop > 300;

      setIsVisible(!isInContact && isPastHero);
    }
  ...
  const href = generateWhatsAppLink(
    ...
    'Hola! Tengo una consulta sobre las clases.'
  );

  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-gray-900/90 backdrop-blur-sm py-3 px-4 hidden lg:block">
